		...
		<table class="tabel2" align="right">
		  <thead>
		    <tr>
		      ...
		      <td style="text-align: center; background: #ddd"><b>ID RETURN</b></td>
...
$hasil_invoice = 	mysqli_query($conn,"SELECT * FROM tb_return a 
        ...
        ?>
          <tr>
            <td style='text-align: center; width:20px'><?php echo $i ?></td>
						<td style='text-align: center; width:70px'><?php echo $data_invoice['id_return'] ?></td>
            <td style='text-align: center; width:70px'><?php echo $data_invoice['id_order'] ?></td>
            <td style='text-align: center; width:80px'><?php echo $data_invoice['tanggal'] ?></td>
						<td style='text-align: center; width:80px'><?php echo $data_invoice['nama_barang'] ?></td>
						<td style='text-align: center; width:80px'><?php echo $data_invoice['username'] ?></td>
						<td style='text-align: center; width:50px'><?php echo $data_invoice['status_return'] ?></td>
						
						<td style='text-align: center; width:40px'><?php echo $data_invoice['jumlah'] ?></td>
						
          </tr>
        <?php $i++; } 
				$query        = mysqli_query($conn,"SELECT sum(jumlah) AS grandtotal FROM tb_return a 
                          JOIN detail_return b ON a.id_return = b.id_return
          ...
          $subtotal     = $data1['grandtotal'];
				...
				?>
				<tr>
            <td style='text-align: center; width:20px' colspan='7'> TOTAL</td>
